...
export default class PolizaServicioModel {
  ...
  static async getByServicio(servicioId) {
    const result = await turso.execute("SELECT * FROM poliza_servicio WHERE  servicio_id = ?",
      [servicioId]
    )
  ...
  static async getByPoliza(polizaId) {
    const result = await turso.execute("SELECT * FROM poliza_servicio WHERE  poliza_id = ?",
      [polizaId]
    )
