...
export default class EmpleadoModel {
  static async getAll() {
    const result = await turso.execute("SELECT * FROM empleado");

    return result.rows;
  }

  static async getById(documento) {
    const result = await turso.execute("SELECT * FROM empleado WHERE documento = ?", [documento]);

    return result.rows[0];
  }
  
  static async getByCorreo(correo) {
    const result = await turso.execute("SELECT * FROM empleado WHERE correo = ?", [correo]);

    return result.rows[0];
  }

  static async getByCorreoAndNotDocumento(correo, documento) {
    const result = await turso.execute("SELECT * FROM empleado WHERE correo = ? AND documento != ?", [correo, documento]);
